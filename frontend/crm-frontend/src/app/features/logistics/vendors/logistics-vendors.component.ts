...
export class LogisticsVendorsComponent implements OnInit {
  ...
  protected readonly selectedVendor =
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.errorMessage.set(
            error?.error?.message ||
            'Unable to load Logistics vendors.'
          );
        }
  ...
  protected saveVendor(): void {
    ...
    const request =
      this.editingId
        ...
        ? this.api.patch<VendorRecord>(
            '/logistics/vendors/' +
            this.editingId,
            payload
          )

        : this.api.post<VendorRecord>(
            '/logistics/vendors',
            payload
          );
        ...
        error: (error) => {
          ...
          const message =
            this.apiErrorMessage(
              error,
              'Unable to save Logistics vendor. Please check backend server and required fields.'
            );
  ...
  private validatePayload(
    ...
    if (
      ...
      !this.form.vendorTypeOther.trim()
    ) {

      return 'Vendor type is required when Other is selected.';
    }
    ...
    if (
      ...
      !this.form.paymentTermOther.trim()
    ) {

      return 'Payment term is required when Other is selected.';
    }
    ...
    if (
      ...
      return 'Vendor status is required when Other is selected.';
    }
  ...
  private buildPayload():
    ...
    return {
      ...
      vendorTypeOther:
        this.form.vendorTypeOther,

      vendorName:
        this.form.vendorName.trim(),
      ...
      paymentTerms:
        this.form.paymentTerm === 'other'
          ? 'other'
          : this.form.paymentTerm,

      paymentTermsOther:
        this.form.paymentTermOther,
      ...
      remarks:
        ...
        'Created from logistics vendor form.'
    ...
    return rows.map(
      (row, index) => {
        ...
        return {
          ...
          /*
           * Preserve complete backend record so existing
           * View/Edit behaviour continues to use all fields.
           */
          raw:
            row
   ...
   * Creator attribution:
   *
   * 1. Prefer Employee because Logistics records are created
   *    from employee workspaces.
   *
   * 2. Fall back to User for legacy / management-created data.
   *
   * 3. Never invent creator information for old records.
   */
