...
interface CustomerRecord {
  ...
  createdBy?: string | CreatorUser | null;
  createdByEmployeeId?: string | CreatorEmployee | null;
...
export class LogisticsCustomersComponent implements OnInit {
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.errorMessage.set(
            error?.error?.message ||
            'Unable to load Logistics customers.'
          );
        }
  ...
  protected saveCustomer(): void {
    ...
    const request =
      this.editingId
        ? this.api.patch<CustomerRecord>(
            '/logistics/customers/' +
              this.editingId,
            payload
          )
        : this.api.post<CustomerRecord>(
            '/logistics/customers',
            payload
          );
        ...
        error: (
          ...
          const message =
            this.apiErrorMessage(
              error,
              'Unable to save Logistics customer.'
            );
  ...
  private validateForm(
    ...
    if (
      this.form.customerType === 'other' &&
      !this.form.customerTypeOther.trim()
    ) {
      return 'Customer Type is required when Other is selected.';
    }
    ...
    if (
      !this.form.preferredMode
    ) {
      return 'Preferred Mode is required.';
    }

    if (
      this.form.preferredMode === 'other' &&
      !this.form.preferredModeOther.trim()
    ) {
      return 'Preferred Mode is required when Other is selected.';
    }

    if (
      this.form.creditTerm === 'other' &&
      !this.form.creditTermOther.trim()
    ) {
      return 'Credit Term is required when Other is selected.';
    }
    ...
    if (
      ...
      return 'Customer Status is required when Other is selected.';
    }
  ...
  protected deleteCustomer(
    ...
    if (
      !id ||
      !window.confirm(
        `Delete customer ${item.customerName}?`
      )
    ) {
      return;
    }
  ...
  protected viewCustomer(
    item: CustomerRecord
  ): void {

    /*
     * Preserve the normalized creator fields together with
     * the original backend record so the details section can
     * display creator information reliably.
     */
    this.selectedCustomer.set({
      ...(item.raw || item),
  ...
  private buildPayload(): Record<string, unknown> {
    ...
    return {
      ...
      customerType:
        this.mapCustomerType(
          this.form.customerType
        ),

      customerTypeOther:
        this.form.customerTypeOther,
      ...
      alternateMobile:
        this.form.alternateMobile.trim(),
      ...
      preferredMode:
        this.mapPreferredMode(
          this.form.preferredMode
        ),

      preferredModeOther:
        this.form.preferredModeOther,
      ...
      remarks:
        ...
        'Created from logistics customer form.'
    };
  }
    ...
    return rows.map(
      (row, index) => {
        ...
        return {
          ...
          /*
           * Important:
           * Keep the complete backend object.
           * Existing edit/view functionality can continue using
  ...
  private resolveCreator(
    ...
    /*
     * Preferred source:
     * Employee record populated from createdByEmployeeId.
     */
    const employee =
      row?.createdByEmployeeId;
    ...
    /*
     * Legacy fallback:
     * Older Customer records may only have createdBy populated.
     */
    const user =
      row?.createdBy;
    ...
    /*
     * Old records can legitimately have no creator employee.
  ...
  private formFromRecord(
    ...
    return {
      ...
      pincode:
        row.billingAddress?.pincode ||
        row.shippingAddress?.pincode ||
        '',

      preferredMode:
        this.preferredModeValue(
          row.preferredMode
        ),

      preferredModeOther:
        row.preferredModeOther ||
        '',
