...
export class EmployeeDashboardComponent implements OnDestroy {
  ...
  protected readonly todayAttendance =
    signal<AttendanceRecord | null>(null);

  protected readonly leaves =
    signal<LeaveRequest[]>([]);

  protected readonly leaveBalances =
    signal<LeaveBalanceRow[]>([]);

  protected readonly employeeBank =
    signal<EmployeeBankDetails | null>(null);

  protected readonly employeeDocuments =
    signal<EmployeeDocumentItem[]>([]);

  protected readonly attendanceHistory =
  ...
  protected readonly employeeRecipients =
    signal<EmployeeRecipientRow[]>([]);
  ...
  protected readonly editingLeadId =
    signal<string | null>(null);

  protected readonly isLeadSaving =
    signal(false);

  protected readonly editingLead =
    computed<CrmLeadRow | null>(() => {
      const leadId =
        this.editingLeadId();
  ...
  protected readonly accountInvoices =
    signal<AccountInvoiceRow[]>([]);

  protected readonly accountPayments =
    signal<AccountPaymentRow[]>([]);

  protected readonly accountExpenses =
    signal<AccountExpenseRow[]>([]);

  protected readonly businessCategories = [
    'Vegetables',
    'Grains',
    'Fruits',
    'Logistics',
  ...
  protected readonly tradeTypes = [
    'Sell',
    'Purchase'
  ] as const;

  protected readonly routeTypes = [
    'Domestic',
    'Export',
    'Import'
  ] as const;
  ...
  protected readonly expenseTypes = [
    'Operations',
    'Purchase',
    'Logistics',
    'Export',
  ...
  protected readonly workTimer =
    signal('00:00:00');

  protected readonly leaveBalance =
    signal(0);
  ...
  protected readonly isRefreshing =
    signal(false);

  protected readonly isPunching =
    signal(false);

  protected readonly isLeaveSaving =
    signal(false);
  ...
  protected readonly isPhotoUploading =
    signal(false);

  protected readonly isPasswordChanging =
    signal(false);

  protected readonly visiblePasswordFields =
    signal<Record<string, boolean>>({});

  protected readonly message =
    signal('');

  protected readonly messagePopup =
    signal('');

  protected readonly activeSection =
    signal<EmployeeFeature>('dashboard');

  protected readonly viewEmployeeId =
    signal<string | null>(null);

  protected readonly viewCompanyId =
    signal<string | null>(null);

  protected readonly historyRange =
    signal<'month' | 'year'>('month');

  protected readonly historyMonth =
    signal(new Date().getMonth() + 1);

  protected readonly historyYear =
    signal(new Date().getFullYear());

  protected readonly holidayMonthFilter =
    signal<number | 'all'>(new Date().getMonth() + 1);
  ...
  protected readonly visibleEmployeeMenu = computed(() => {
    const visible =
      this.employeeMenu.filter(
        (item) =>
          this.canShowMenuItem(item.id)
      );
    ...
    const salesOrder: EmployeeFeature[] = [
      'dashboard',
      'my-leads',
      'my-deals',
      'my-tasks',
  ...
  protected readonly salesWorkspaceMenu =
    computed<FeatureItem[]>(() =>
      this.visibleEmployeeMenu().filter(
        (item) =>
          [
            'dashboard',
            'my-leads',
  ...
  protected readonly employeeSelfServiceMenu =
    computed<FeatureItem[]>(() =>
      this.visibleEmployeeMenu().filter(
        (item) =>
          ![
  ...
  protected readonly logisticsMenu: LogisticsMenuItem[] = [
    ...
    },
    {
      label: 'New Air Cargo',
      icon: 'A',
      route: '/logistics/air-cargo/new'
    },
    {
      label: 'All Air Cargo',
      icon: 'L',
      route: '/logistics/air-cargo'
    },
    {
      label: 'Sea Freight',
      icon: 'S',
      route: '/logistics/sea-freight'
    },
    {
      label: 'CHA / Customs',
      icon: 'C',
    ...
    },
    {
      label: 'Customers',
      icon: 'C',
      route: '/logistics/customers'
    },
    {
      label: 'Vendors',
      icon: 'V',
      route: '/logistics/vendors'
    },
    {
      label: 'Products / Services',
      icon: 'P',
      route: '/logistics/products-services'
    },
    {
  ...
  constructor() {
    this.viewEmployeeId.set(
      this.route.snapshot.queryParamMap.get(
        'employeeId'
      )
    );

    this.viewCompanyId.set(
      this.route.snapshot.queryParamMap.get(
        'companyId'
      )
    );
    ...
    if (
      !this.isPreviewMode() &&
      !this.isLogisticsEmbedded() &&
      this.auth.isLogisticsUser()
    ) {
  ...
  protected refresh(
    ...
    forkJoin({
      ...
      recipients:
        this.api
          .get<{
            employees?: EmployeeRecipientRow[];
          }>(
            '/hr/communication/messages/employee-recipients',
            {
              limit: 40
            }
          )
      ...
      .subscribe(
        ({
          ...
          this.loadEmployeeExtras(
            dashboard?.employee?._id
          );
  ...
  protected createLead(): void {
    ...
    if (
      ...
      this.message.set(
        'Please enter the custom lead source.'
      );
    ...
    const request =
      leadId
        ? this.api.patch<CrmLeadRow>(
            `/crm/leads/${leadId}`,
            payload
          )
        : this.api.post<CrmLeadRow>(
            '/crm/leads',
            payload
          );
  ...
  protected startLeadEdit(
    ...
    if (!leadId) {
      this.message.set(
        'Unable to edit this lead because its record ID is missing.'
      );

      this.showMessagePopup(
        'Unable to edit this lead.'
      );
    ...
    /*
     * Patch the form immediately from the list row.
     * This keeps Edit working even when the backend detail
     * endpoint is unavailable or responds slowly.
     */
    this.editingLeadId.set(
      leadId
    );
    ...
    this.message.set(
      `Editing ${lead.name || 'lead'}. Update the fields and click Save Lead.`
    );

    this.scrollLeadEditorIntoView();

    /*
     * Enrich the editor with the latest server copy when the
     * detail endpoint is available. A failed detail request
     * must never cancel the edit state that already works
     * from the table/list row.
     */
    this.api
      .get<CrmLeadRow>(
        `/crm/leads/${leadId}`
  ...
  private patchLeadForm(
    row: CrmLeadRow
  ): void {
    const knownSource =
      this.isKnownLeadSource(
        row.source
      );
  ...
  private scrollLeadEditorIntoView(): void {
    setTimeout(
      () => {
        const editor =
  ...
  private scrollLeadListIntoView(): void {
    setTimeout(
      () => {
        const list =
          document.getElementById(
  ...
  protected leadEditorTitle(): string {
    const lead =
      this.editingLead();

    return lead
      ? `Edit Lead: ${lead.name || 'Lead'}`
      : 'Add New Lead';
  }

  protected leadEditorSubtitle(): string {
    return this.editingLeadId()
      ? 'Update the selected lead details and save your changes.'
      : 'Capture a new lead and keep the sales pipeline up to date.';
  }

  protected leadEditorActionLabel(): string {
    if (this.isLeadSaving()) {
      return this.editingLeadId()
        ? 'Saving Changes...'
        : 'Adding Lead...';
    }
  ...
  protected workspaceEyebrow(): string {
    return this.isSalesDepartment()
      ? 'My Sales Workspace'
      : 'My Workspace';
  }

  protected workspaceTitle(): string {
    return this.isSalesDepartment()
      ? 'Sales Dashboard'
      : 'Employee Dashboard';
  }

  protected workspaceDescription(): string {
    if (this.isSalesDepartment()) {
      return `${this.companyName()} sales pipeline, leads, follow-ups and employee self-service for ${this.userName()}.`;
    }

    return `${this.companyName()} attendance, leave, notifications and HR updates for ${this.userName()}.`;
  }

  protected salesLeadCount(): number {
    return this.crmLeads().length;
  }

  protected salesActiveLeadCount(): number {
  ...
  protected salesActiveDealCount(): number {
    return this.crmDeals()
      .filter(
        (deal) =>
          ![
  ...
  protected salesWonDealCount(): number {
    return this.crmDeals()
      .filter(
        (deal) =>
          String(
  ...
  protected salesWonRevenue(): number {
    return this.crmDeals()
      .filter(
        (deal) =>
          String(
  ...
  protected salesDueTodayTaskCount(): number {
    const today =
      this.toDateInput(
        new Date()
      );
  ...
  protected salesOverdueTaskCount(): number {
    const today =
      this.toDateInput(
        new Date()
      );
  ...
  protected salesQualifiedLeadCount(): number {
    return this.crmLeads()
      .filter(
        (lead) =>
          [
  ...
  protected salesConversionPercent(): number {
    const total =
      this.salesLeadCount();
  ...
  protected salesRecentRows(): Array<{
    title: string;
    meta: string;
    status: string;
    date?: string;
  ...
  protected isLeadSourceOther(): boolean {
  ...
  private loadEmployeeExtras(
    ...
    forkJoin({
      bank:
        this.api
          .get<EmployeeBankDetails | null>(
            `/hr/employees/${employeeId}/bank`
          )
          .pipe(
            catchError(
              () =>
      ...
      documents:
        this.api
          .get<EmployeeDocumentsResponse | null>(
  ...
  protected setActiveSection(
    section:
      EmployeeFeature
  ): void {
    if (
      !this.canAccessSection(
        section
      )
    ) {
      this.message.set(
        'This option is not available for your department.'
      );
  ...
  protected toggleNotificationPanel(): void {
    this.isNotificationPanelOpen.update(
      (value) =>
        !value
    );
  ...
  protected setHistoryRange(
    value:
      'month' |
      'year'
  ): void {
    this.historyRange.set(
      value
    );
  ...
  protected setHistoryMonth(
    value:
      string
  ): void {
    this.historyMonth.set(
      Number(value)
    );
  ...
  protected setHolidayMonthFilter(
    value:
      string
  ): void {
    this.holidayMonthFilter.set(
      value ===
      'all'
        ? 'all'
        : Number(value)
  ...
  protected setHistoryYear(
    value:
      string
  ): void {
    this.historyYear.set(
      Number(value)
    );
  ...
  protected leaveBalanceByCode(
    ...
    return balance
      ? this.leaveBalanceAvailable(
          balance
        )
      : 0;
  }

  protected attendanceTrend(): Array<{
    label: string;
    status: string;
  }> {
    return this.attendanceHistory()
  ...
  protected leaveHistoryBars(): Array<{
    label: string;
    value: number;
    percent: number;
  }> {
  ...
  protected recentActivity(): Array<{
    title: string;
    meta: string;
  }> {
    const leaveUpdates =
      this.leaves()
        ...
        .map(
          (leave) => ({
            ...
            meta:
              `${leave.leaveTypeId?.leaveName || 'Leave'} | ${this.formatDate(leave.fromDate)}`
          })
        );
  ...
  protected leaveCodeOptions(): Array<{
    code: string;
    label: string;
  }> {
    const options =
  ...
  protected leaveBalanceAvailable(
    balance:
      LeaveBalanceRow
  ): number {
    const available =
  ...
  protected leaveBalanceRemainingText(
    balance:
      LeaveBalanceRow
  ): string {
  ...
  protected applyLeave(): void {
    ...
    if (!selectedBalance) {
      this.message.set(
        'Selected leave type is not assigned to you.'
      );
      return;
    }
    ...
    this.api
      ...
      .pipe(
        finalize(
          () =>
            this.isLeaveSaving.set(false)
        )
      )
      .subscribe({
        next: () => {
          this.message.set(
            'Leave applied. HR has been notified.'
          );
  ...
  protected async uploadEmployeePhoto(
    event:
      Event
  ): Promise<void> {
    const input =
    ...
    if (
      ...
      this.message.set(
        'Profile photo source must be 10 MB or smaller.'
      );
    ...
    try {
      croppedFile =
        await this.imageCropper.cropImage(
          file,
          {
            title:
              'Crop profile photo',
    ...
    if (
      ...
      this.message.set(
        'Cropped profile photo must be 2 MB or smaller.'
      );
  ...
  protected isPasswordFieldVisible(
    field:
      string
  ): boolean {
    return Boolean(
      this.visiblePasswordFields()[
        field
      ]
    );
  }

  protected togglePasswordField(
    field:
      string
  ): void {
    this.visiblePasswordFields.update(
      (fields) => ({
        ...fields,
  ...
  protected attendanceDeviceLabels(): string {
    const devices =
      this.normalizeAttendanceDevices(
        this.dashboard()
          ?.employee
          ?.attendanceAllowedDevices
      );
  ...
  private normalizeAttendanceDevices(
    value:
      unknown
  ): string[] {
    const allowed = [
  ...
  protected isSalesDepartment(): boolean {
    return (
      this.departmentAccessModules()
        .includes(
          'sales-crm'
        ) ||
      this.departmentFeatureKey() ===
        'sales' ||
      this.isDepartmentMatch(
        /sales/i
      )
    );
  }
  ...
  private canShowMenuItem(
    ...
    if (
      ...
      return this.isAccountsDepartment();
    }

    return true;
  }

  private canAccessSection(
    section:
      EmployeeFeature
  ): boolean {
    return this.canShowMenuItem(
      section
    );
  }
  ...
  protected payslipPdfUrl(
    ...
    return slip.pdfUrl
      ...
      : apiUrl(
          `/hr/payroll/payslips/${slip._id}/pdf`
        );
  }

  protected downloadPayslipPdf(
    slip:
      PayslipRow
  ): void {
    const url =
    ...
    this.api
      ...
      .subscribe({
        ...
        error: () =>
          this.message.set(
            'Unable to download payslip. Please login again and try.'
          )
      });
  }
  ...
  protected selectedMeetingInviteCount(): number {
    return this.normalizeSelectedInviteKeys(
      this.meetingForm
        .controls
        .attendeeInviteKeys
        .value
  ...
  protected isMeetingInviteSelected(
    ...
    return (
      !!key &&
      this.normalizeSelectedInviteKeys(
        this.meetingForm
          .controls
          .attendeeInviteKeys
          .value
    ...
    this.api
      .post<MeetingRow>(
        '/hr/meetings',
        {
          ...
          attendees:
            this.meetingAttendeePayload(
              raw.attendeeInviteKeys
            ),
      ...
      .subscribe({
        next: () => {
          this.message.set(
            'Meeting room created and invite notifications sent.'
          );
  ...
  private normalizeSelectedInviteKeys(
    value:
      unknown
  ): string[] {
  ...
  protected meetingInviteKey(
    recipient:
      EmployeeRecipientRow
  ): string {
    if (
      recipient.inviteType ===
        'user' ||
  ...
  private meetingAttendeePayload(
    value:
      unknown
  ): Array<{
    employeeCode?: string;
  ...
  protected openLogisticsRoute(
    ...
    if (
      ...
      this.message.set(
        'Logistics workspace is not available for your department.'
      );
      return;
    }
    ...
    return logs
      .map(
        (
          ...
          `${index + 1}. ${this.formatTime(log.checkInTime)} - ${this.formatTime(log.checkOutTime)}`
      )
      .join(
        ' | '
      );
  ...
  protected holidayMonthLabel(): string {
    return this.holidayMonthFilter() ===
      'all'
      ? 'All months'
      : new Intl.DateTimeFormat(
          'en-IN',
  ...
  private applyNotifications(
    ...
    if (incoming) {
      this.showMessagePopup(
        `${incoming.title || 'Notification'}: ${incoming.message || 'New update'}`
      );
    }
  }

  private markMessageNotificationsSeen(
    rows =
      this.messages()
  ): void {
    const currentUserEmail =
      ...
      forkJoin(
        unreadIncoming.map(
          (item) =>
            this.api
              .patch<MessageRow>(
                `/hr/communication/messages/${item._id}/read`,
                {}
              )
              .pipe(
                catchError(
  ...
  private currentCompanyFallback(): {
    name?: string;
    logoUrl?: string;
    settings?: {
      theme?: CompanyTheme;
